...
カスタムテンプレートを使用してLab環境をデプロイします。Key Vaultを作成してキーおよびシークレットを作成します。アプリケーションの登録をし、クライアントシークレットを作成します。VMからSSMSを起動し、データベースにAlways Encryptedを設定します。 Visual Studioを使用して暗号化したデータをアプリケーションから復号して表示します。
